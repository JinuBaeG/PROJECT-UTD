...
namespace UTD
{
    public class SelectionUI : MonoBehaviour
    {
        ...
        private List<SelectableCharacter> selectableCharacters = new List<SelectableCharacter>();
        ...
        private void Update()
        {
            if (Input.GetKeyDown(KeyCode.Mouse1)) // Right Mouse Button Click
            {
                ...
                if(Physics.Raycast(ray, out RaycastHit hit, 1000f))
                {
                    ...
                    {
                        selectableCharacters[i].SetDestination(hit.point);
                    }
                }
